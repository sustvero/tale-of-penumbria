...
'''
ISSUES TO FIX IF I HAVE TIME:
- Shaking when on platforms --> minimized, not fixed
- being "glued" to bottom of platforms 
'''
...
class Fireball(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, image):
        ...
        self.y = y
        self.image = image
        self.rect = pygame.Rect(x + 5, y + 60, 20, 20)
        self.rect.y = y
        self.rect.x = x
...
#player collision with platform
    ...
    for plat in plat_list:
        #left side:
        if plat.rect.x - 4 <= char.x + 30 <= plat.rect.x + 4 : #if player's right side is in the same place as the left side of the platform:
            if (plat.rect.y <= char.y + 15 <= plat.rect.y + 40)  or (plat.rect.y <= char.y + 55 <= plat.rect.y + 45): #if the top or the bottom of the player is in contact with the platform:
                char.x -= char.vel
                collideLeft = True
                char.Right = False
        #right side
        elif plat.rect.x - 4 <= char.x - 15 <= plat.rect.x + 1:
            if (plat.rect.y <= char.y + 15 <= plat.rect.y + 40)  or (plat.rect.y <= char.y + 55 <= plat.rect.y + 45):
                char.x += char.vel
                collideRight = True
                char.Left = False
     #top side
        if plat.rect.y - 8 <= char.y + 55 <= plat.rect.y + 8:
            if (plat.rect.x  <=  char.x + 35  <= plat.rect.x + 40) or (plat.rect.x <= char.x + 30 <= plat.rect.x + 45):
                grav = -0.1
                grav -= 0.1
                char.on_plat = True
    #bottom side
        if plat.rect.y <=  char.y + 15 <= plat.rect.y + 42:
             if (plat.rect.x  <=  char.x + 35  <= plat.rect.x + 40) or (plat.rect.x <= char.x + 30 <= plat.rect.x + 45):
                grav = 0
                grav += 0.35
#collision with energy
    for energy in energy_list:
        #left side:
        if energy.rect.x <= char.x + 30 <= energy.rect.x + 20 or energy.rect.x <= char.x <= energy.rect.x + 20: #if player's right side is in the same place as the left side of the platform:
            if (energy.rect.y <= char.y + 17 <= energy.rect.y + 12)  or  (energy.rect.y <= char.y + 34 <= energy.rect.y + 12) or (energy.rect.y <= char.y + 55 <= energy.rect.y + 12): #if the top or the bottom of the player is in contact with the platform:
                if health < 140 : health += 10
                else: health = 140
                energy_list.remove(energy) #you've collected it!
#collision with spikes!
    for s in spike_list:
        if s.rect.y <= char.y + 55 <= s.rect.y + 40:
            if (s.rect.x  <=  char.x + 35 <= s.rect.x + 40):
                health -= 1
#collision with fireball
    for fire in fire_list:
        if fire.rect.y <= char.y + 15 <= fire.rect.y + 20 or fire.rect.y <= char.y + 32 <= fire.rect.y + 25 or fire.rect.y <= char.y + 45 <= fire.rect.y + 20:
            if fire.rect.x  <=  char.x + 35 <= fire.rect.x + 20 or fire.rect.x < char.x <= fire.rect.x + 20:
                health -= 40
                fire_list.remove(fire)
#collision with door
    for door in portal_list:
        if door.rect.x  <= char.x  <= door.rect.x + 80:
            if door.rect.y <=char.y + 15 <= door.rect.y + 80:
                print("hello!")
                win = True
